      ...
      <div className="max-w-[1440px] mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
          {pricingData.map((plan, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className={`p-10 border border-dust-grey/10 rounded-xs flex flex-col items-start transition-colors group bg-carbon-black/10 ${i === 0 ? "md:col-span-2 xl:col-span-1" : "md:col-span-1"}`}>
              ...
              <ul className={`grid gap-x-12 gap-y-4 mb-12 w-full flex-grow ${i === 0 ? "md:grid-cols-2 xl:grid-cols-1" : "grid-cols-1"}`}>
